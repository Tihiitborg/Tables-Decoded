...
class TableVQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # append <END> token to gold answer
        answer = entry.get("answer_text", "").strip() + f" {self.end_token}"
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            for i in range(prompt_ids.size(0)):
                ...
                pred = tokenizer.decode(generated, skip_special_tokens=True).lower().strip()

                # cut off at <END>
                if end_token.lower() in pred:
                    pred = pred.split(end_token.lower())[0].strip()

                gold = answers[i].lower().strip()
                if end_token.lower() in gold:
                    gold = gold.replace(end_token.lower(), "").strip()

                lev_score = Levenshtein.ratio(pred, gold)
                ...
                if lev_score >= threshold:
                    lev_sim += 1
                if pred == gold or gold in pred or lev_score >= threshold:
                    relieved += 1

                total += 1
                print(f"Q: {questions[i]}")
                print(f"Pred: {pred} | Gold: {gold}")
                print(f"Exact Match: {int(pred == gold)} | Levenshtein Score: {lev_score:.3f}\n")
...
def main():
    ...
    predictions_save = "solar10b_official_test_predictions.json"
    ...
    # Load OFF-THE-SHELF MODEL (no checkpoint loading!)
