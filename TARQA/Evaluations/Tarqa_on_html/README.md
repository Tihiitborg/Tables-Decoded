# 📑 Supervised Fine-Tuning (SFT) on TARQA with HTML Tables

This project applies **Supervised Fine-Tuning (SFT)** to adapt a pretrained LLaMA model (**Meta-LLaMA-3-8B-Instruct**) for the **Table Visual Question Answering (TableVQA)** task on the **TARQA dataset**.  
The goal is to train the model to generate **short, accurate answers** given **HTML tables** and **natural language questions**.

---

## 🎯 Goal
- Adapt the pretrained model to perform **structured reasoning over HTML tables**.  
- **Training objective:** predict the correct answer text given a serialized HTML table + natural language question.  
...
- An instruction  
- A table in **serialized HTML** format  
...
### Steps:
1. **Forward Pass**  
   - Input = tokenized HTML + question + answer  
