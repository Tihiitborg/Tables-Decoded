...
with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
    f.write("# PaliGemma Evaluation Results\n")
...
for sample in pbar:
    ...
    question = f"<image> संक्षेप में, केवल प्रत्यक्ष उत्तर दें। {sample['question']}"
    ...
    try:
        ...
        if question in predicted_answer:
            predicted_answer = predicted_answer.replace(question, "").strip()

        # For answers after newline (common), pick the last line
        if '\n' in predicted_answer:
            predicted_answer = predicted_answer.split('\n')[-1].strip()

        # Remove leading punctuation (optional, but helps)
        predicted_answer = predicted_answer.lstrip(":।-—–\n\t .")
